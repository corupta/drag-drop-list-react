...
Use `npm i -g react-drag-drop` to install globally, or `npm i -D react-drag-drop` to install it locally to your project.

Make sure to install its dependencies: `react`, `prop-types` and `react-dom`.
...
Give a unique id to a list. This field is required. Note that, `insertItem` and `removeItem` functions are called
